...
const Issues = () => {
  ...
  return <>
   <ToastContainer
              ...
              transition={Bounce}
          />
    <div className="absolute inset-0 -z-10 h-full w-full px-2 py-12 [background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#63e_100%)] items-center">
  <div className="flex justify-center items-start mt-10 bg-transparent max-h-[80vh] max-w-[90vw] overflow-auto">
    <table className="w-full max-w-[95vw] md:max-w-[80vw] lg:max-w-[60vw] mx-auto divide-y divide-x divide-gray-100 border border-gray-200 bg-transparent text-white z-20">
      <thead>
        <tr>
        ...
        {userdata.length === 0 ? (
          ...
          userdata.map((problem, index) => (
            <tr key={index}>
              ...
              <td className="px-2 py-2 align-top text-left whitespace-normal break-words text-xs md:text-sm font-normal text-white underline">
              ...
              <td className="px-2 py-2 text-right text-xs md:text-sm font-normal text-black">
                <button
                  className="hover:cursor-pointer relative inline-flex items-center justify-center p-0.5 mb-1 overflow-hidden text-xs md:text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 hover:text-white focus:ring-2 focus:outline-none focus:ring-blue-300"
